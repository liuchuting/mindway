...
from transformers.models.convbert.configuration_convbert import ConvBertConfig
...
def load_tf_weights_in_convbert(model, config, tf_checkpoint_path):
    ...
    for param in model.named_parameters():
        ...
        value = ms.from_numpy(tf_data[tf_name])
...
class ConvBertSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        conv_kernel_layer = mint.reshape(conv_kernel_layer, [-1, self.conv_kernel_size, 1])
        conv_kernel_layer = mint.softmax(conv_kernel_layer, dim=1)
        ...
        conv_out_layer = mint.reshape(conv_out_layer, [-1, self.attention_head_size, self.conv_kernel_size])
        conv_out_layer = mint.matmul(conv_out_layer, conv_kernel_layer)
        conv_out_layer = mint.reshape(conv_out_layer, [-1, self.all_head_size])
        ...
        conv_out = mint.reshape(conv_out_layer, [batch_size, -1, self.num_attention_heads, self.attention_head_size])
        context_layer = mint.cat([context_layer, conv_out], 2)
...
class GroupedLinearLayer(nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        x = mint.reshape(hidden_states, [-1, self.num_groups, self.group_in_dim])
        x = x.permute(1, 0, 2)
        x = mint.matmul(x, self.weight)
        x = x.permute(1, 0, 2)
        x = mint.reshape(x, [batch_size, -1, self.output_size])
        x = x + self.bias
        return x
...
class ConvBertEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.CellList([ConvBertLayer(config) for _ in range(config.num_hidden_layers)])
...
CONVBERT_START_DOCSTRING = r"""
    ...
    it as a regular PyTorch Module and refer to the MindSpore documentation for all matter related to general usage and
    behavior.
