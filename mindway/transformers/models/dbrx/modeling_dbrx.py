...
class DbrxRotaryEmbedding(nn.Cell):
...
def load_balancing_loss_func(
    gate_logits: ms.Tensor,
    num_experts: int,
    top_k: int,
    attention_mask: Optional[ms.Tensor],
) -> ms.Tensor:
    r"""Computes auxiliary load balancing loss as in Switch Transformer - implemented in MindSpore.
    ...
    Args:
        gate_logits (Union[`ms.Tensor`, Tuple[ms.Tensor]):
...
class DbrxAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Any,
        ...
        attn_weights = mint.nn.functional.dropout(attn_weights, p=self.attn_pdrop, training=self.training)
...
class DbrxNormAttentionNorm(nn.Cell):
    def __init__(self, config: DbrxConfig, block_idx: Optional[int] = None):
        super().__init__()
        self.block_idx = block_idx
        self.resid_pdrop = config.resid_pdrop
        self.norm_1 = mint.nn.LayerNorm(config.d_model, bias=False)
        self.attn = DBRX_ATTENTION_CLASSES[config._attn_implementation](
            config=config,
            block_idx=block_idx,
        )
        self.norm_2 = mint.nn.LayerNorm(config.d_model, bias=False)
    ...
    def construct(
        ...
        cache_position: Optional[ms.Tensor] = None,
        **kwargs: Any,
    ) -> Tuple[ms.Tensor, ms.Tensor, Optional[ms.Tensor], Optional[Cache]]:
        ...
        hidden_states = mint.nn.functional.dropout(hidden_states, p=self.resid_pdrop, training=self.training)
...
class DbrxRouter(nn.Cell):
    def __init__(
        ...
        self.layer = mint.nn.Linear(self.hidden_size, self.moe_num_experts, bias=False)

    def construct(self, hidden_states: ms.Tensor) -> Tuple[ms.Tensor, ms.Tensor, ms.Tensor]:
        if self.training and self.moe_jitter_eps is not None:
            hidden_states *= mint.empty_like(hidden_states).uniform_(
        ...
        top_weights_scale = (
            mint.norm(top_weights, p=self.moe_normalize_expert_weights, dim=-1, keepdim=True)
...
class DbrxExpertGLU(nn.Cell):
    def __init__(self, hidden_size: int, ffn_hidden_size: int, moe_num_experts: int, ffn_act_fn: dict):
        ...
        self.w1 = ms.Parameter(mint.empty(moe_num_experts * ffn_hidden_size, hidden_size))
        self.v1 = ms.Parameter(mint.empty(moe_num_experts * ffn_hidden_size, hidden_size))
        self.w2 = ms.Parameter(mint.empty(moe_num_experts * ffn_hidden_size, hidden_size))

        act_fn_name = ffn_act_fn.get("name", "silu")
        self.activation_fn = ACT2FN[act_fn_name]

    def construct(
        self, x: ms.Tensor, expert_w1: ms.Tensor, expert_v1: ms.Tensor, expert_w2: ms.Tensor
    ) -> ms.Tensor:
...
class DbrxExperts(nn.Cell):
    ...
    def construct(
        self, x: ms.Tensor, weights: ms.Tensor, top_weights: ms.Tensor, top_experts: ms.Tensor
    ) -> ms.Tensor:
        bsz, q_len, hidden_size = x.shape
        x = x.view(-1, hidden_size)
        out = mint.zeros_like(x)

        expert_mask = mint.nn.functional.one_hot(top_experts, num_classes=self.moe_num_experts).permute(2, 1, 0)
        ...
        for expert_idx in range(0, self.moe_num_experts):
            ...
            topk_idx, token_idx = mint.where(expert_mask[expert_idx])
...
class DbrxBlock(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Any,
        ...
        Tuple[ms.Tensor, Optional[Cache], Optional[ms.Tensor]],
        Tuple[ms.Tensor, Optional[ms.Tensor], Optional[Cache], Optional[ms.Tensor]],
    ]:
        """Forward function for DbrxBlock.

        Args:
            hidden_states (`ms.Tensor`): input to the layer of shape `(batch, seq_len, embed_dim)`
            position_ids (`ms.Tensor`): position ids of shape `(batch, seq_len)`
            ...
            cache_position (`ms.Tensor`, *optional*): position ids of the cache
        """
...
class DbrxModel(DbrxPreTrainedModel):
    ...
    def __init__(self, config: DbrxConfig):
        ...
        self.wte = mint.nn.Embedding(config.vocab_size, config.d_model, self.padding_idx)
        self.blocks = nn.CellList([DbrxBlock(config, block_idx) for block_idx in range(config.n_layers)])
        self.norm_f = mint.nn.LayerNorm(config.d_model, bias=False)
    ...
    def construct(
        ...
        **kwargs,  # NOOP kwargs, for now
        ...
        inputs_embeds = mint.nn.functional.dropout(inputs_embeds, p=self.emb_pdrop, training=self.training)
        ...
        **kwargs,
        ...
        if output_router_logits:
            ...
            if labels is not None and loss is not None:
                loss += self.moe_loss_weight * aux_loss
