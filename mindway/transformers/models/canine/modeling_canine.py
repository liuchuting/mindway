...
class CanineModelOutputWithPooling(ModelOutput):
    ...
    Args:
        ...
        hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `ms.Tensor` (one for the input to each encoder + one for the output of each layer of each
        ...
        attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer) of the 3 Transformer encoders of shape `(batch_size,
...
class CanineEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        for i in range(config.num_hash_functions):
            ...
            setattr(self, name, mint.nn.Embedding(config.num_hash_buckets, shard_embedding_size))
        self.char_position_embeddings = mint.nn.Embedding(config.num_hash_buckets, config.hidden_size)
    ...
    def _embed_hash_buckets(self, input_ids, embedding_size: int, num_hashes: int, num_buckets: int):
        ...
        return mint.cat(embedding_shards, dim=-1)
...
class CharactersToMolecules(nn.Cell):
    ...
    def construct(self, char_encoding: ms.Tensor) -> ms.Tensor:
        ...
        char_encoding = mint.transpose(char_encoding, 1, 2)
        downsampled = self.conv(char_encoding)
        downsampled = mint.transpose(downsampled, 1, 2)
        ...
        result = mint.cat([cls_encoding, downsampled_truncated], dim=1)
...
class ConvProjection(nn.Cell):
    ...
    def construct(
        self,
        inputs: ms.Tensor,
        final_seq_char_positions: Optional[ms.Tensor] = None,
    ) -> ms.Tensor:
        ...
        inputs = mint.transpose(inputs, 1, 2)
        ...
        pad = mint.nn.ConstantPad1d((pad_beg, pad_end), 0)
        # `result`: shape (batch_size, char_seq_len, hidden_size)
        result = self.conv(pad(inputs))
        result = mint.transpose(result, 1, 2)
    ...
    def construct(
        ...
        if attention_mask is not None:
            if attention_mask.ndim == 3:
                ...
                attention_mask = mint.unsqueeze(attention_mask, dim=1)
                # Since attention_mask is 1.0 for positions we want to attend and 0.0 for
                # masked positions, this operation will create a tensor which is 0.0 for
                # positions we want to attend and the dtype's smallest value for masked positions.
                attention_mask = (1.0 - attention_mask.float()) * dtype_to_min(attention_scores.dtype)
...
class CanineSelfOutput(nn.Cell):
    ...
    def construct(
        self, hidden_states: Tuple[ms.Tensor], input_tensor: ms.Tensor
        ...
        if not self.local:
            ...
            for (from_start, from_end), (to_start, to_end) in zip(from_chunks, to_chunks):
                ...
                if self.always_attend_to_first_position:
                    ...
                    attention_mask_chunk = mint.cat([cls_attention_mask, attention_mask_chunk], dim=2)

                    cls_position = to_tensor[:, 0:1, :]
                    to_tensor_chunk = mint.cat([cls_position, to_tensor_chunk], dim=1)
            ...
            attention_output = mint.cat(attention_output_chunks, dim=1)
...
class CanineOutput(nn.Cell):
    ...
    def construct(self, hidden_states: Tuple[ms.Tensor], input_tensor: ms.Tensor) -> ms.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states
...
class CaninePooler(nn.Cell):
    ...
    def construct(self, hidden_states: Tuple[ms.Tensor]) -> ms.Tensor:
...
class CanineOnlyMLMHead(nn.Cell):
    ...
    def construct(
        self,
        sequence_output: Tuple[ms.Tensor],
    ) -> Tuple[ms.Tensor]:
...
CANINE_START_DOCSTRING = r"""
    This model is a MindSpore [ms.nn.Cell](https://www.mindspore.cn/docs/zh-CN/master/api_python/nn/mindspore.nn.Cell.html?highlight=cell#mindspore.nn.Cell) sub-class. Use
...
class CanineModel(CaninePreTrainedModel):
    ...
    def _create_3d_attention_mask_from_input_mask(self, from_tensor, to_mask):
        ...
        to_mask = mint.reshape(to_mask, (batch_size, 1, to_seq_length)).float()

        # We don't assume that `from_tensor` is a mask (although it could be). We
        # don't actually care if we attend *from* padding tokens (only *to* padding)
        # tokens so we create a tensor of all ones.
        broadcast_ones = mint.ones(size=(batch_size, from_seq_length, 1), dtype=ms.float32)
    ...
    def _downsample_attention_mask(self, char_attention_mask: ms.Tensor, downsampling_rate: int):
        ...
        molecule_attention_mask = mint.squeeze(pooled_molecule_mask, dim=-1)

        return molecule_attention_mask

    def _repeat_molecules(self, molecules: ms.Tensor, char_seq_length: ms.Tensor) -> ms.Tensor:
        ...
        repeated = mint.repeat_interleave(molecules_without_extra_cls, repeats=rate, dim=-2)
        ...
        remainder_length = mint.fmod(ms.Tensor(char_seq_length), ms.Tensor(rate)).item()
        remainder_repeated = mint.repeat_interleave(
            last_molecule,
        ...
        return mint.cat([repeated, remainder_repeated], dim=-2)
    ...
    def construct(
        ...
        extended_molecule_attention_mask: ms.Tensor = self.get_extended_attention_mask(
        ...
        concat = mint.cat([input_char_encoding, repeated_molecules], dim=-1)
